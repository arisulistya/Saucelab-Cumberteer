...
When('I input invalid user credentials and click login', async function(){
    const loginPage = new LoginPage(this.page);
    await loginPage.loginToApp(process.env.INVALID_USER, process.env.DEFAULT_PASS);
});
...
// Base function for login
Given('Success logged in as valid user', async function(){
    let baseUrl;
    switch (process.env.NODE_ENV) {
        case 'test':
            baseUrl = process.env.TEST_URL;
